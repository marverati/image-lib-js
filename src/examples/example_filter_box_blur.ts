...
export function boxBlurGrayscale(map: GrayscalePixelMap, dx: number, dy = dx): GrayscalePixelMap {
    let sums: number[][] = [[]];
    // Top left pixel
    sums[0][0] = map.get(0, 0);
    // Top row
    for (let x = 1; x < map.width; x++) {
        const prev = sums[0][x - 1];
        const current = map.get(x, 0);
        sums[0][x] = prev + current;
    }
    // Left column
    for (let y = 1; y < map.height; y++) {
        const prev = sums[y - 1][0];
    ...
    for (let y = 1; y < map.height; y++) {
        for (let x = 1; x < map.width; x++) {
            ...
            const topleft = sums[y - 1][x - 1];
            const current = map.get(x, y);
            sums[y][x] = top + left - topleft + current;
        }
    }
    ...
    const offX2 = dx - offX;
    const offY2 = dy - offY;
    const blurMap = ImageLib.filter(map, (c: number, x: number, y: number) => {
        const x1 = Math.max(0, x - offX), y1 = Math.max(0, y - offY);
        const x2 = Math.min(map.width - 1, x + offX2), y2 = Math.min(map.height - 1, y + offY2);
        const pixels = (x2 - x1) * (y2 - y1);
        return (sums[y2][x2] - sums[y2][x1] - sums[y1][x2] + sums[y1][x1]) / pixels;
    });
    return blurMap;
}
